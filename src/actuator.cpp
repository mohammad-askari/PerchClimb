...
void Actuator::init(uint8_t pin, int8_t offset, int8_t range,
...
void Actuator::setRange(int8_t range)
{
  this->range = range;
  setLimits();
  move();
}
void Actuator::setOffset(int8_t offset)
{
  this->offset = offset;
  setLimits();
  move();
}
void Actuator::setFrequency(int8_t frequency)
{
  this->frequency = frequency;
  move();
}
void Actuator::setPosition(int8_t position)
{
...
void Actuator::updatePosition()
{
  ...
  float a = (max_pos - min_pos) / half_period;
  float b = 2.0 * max_pos - min_pos;
  ...
  switch (mode)
  {
    ...
    case LINEAR: {
      if (frequency >= 0) {
        position = is_first_half ? a * dt + min_pos : -a * dt + b;
      } else {
        position = is_first_half ? -a * dt + b : a * dt + min_pos;
      }
      break;
    }
  }
